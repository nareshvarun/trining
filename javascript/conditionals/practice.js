...
console.log(passwordStrength("nareshvarun"));
...
console.log(firstInDictionary('whale','zebra', 'yak'));
...
var getTagName = function (raju) {
  if(raju.slice(raju.indexOf('<')+1,raju.indexOf('>')) ===
  raju.slice(raju.lastIndexOf('<')+2,raju.lastIndexOf('>'))){
return raju.slice(raju.indexOf('<')+1,raju.indexOf('>'));
}
else{
  return "not html"
}
};
console.log(getTagName("<div>nareshvarun</p>"));
...
// practice problem.
var isQuestion = function (varun) {
  var QUESTION="?";
  var last_element=varun.charAt(varun.length-1);
  return last_element== QUESTION;
};
console.log(isQuestion("what?"));
