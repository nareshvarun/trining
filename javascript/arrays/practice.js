...
//     //=> false
//
var containsTwice2 = function (a,array) {
  var counter = 0;
...
console.log(containsNTimes(3,"hello",["hello","hello","hello"]));
...
//
var atLeastOneEven = function (kick) {
  if(Array.isArray(kick)){
    return kick.some(function(item){
      if(item%2==0)
      return true;
    })
  }
...
console.log(atLeastOneEven([101,203,401,2]));
...
console.log(allStrings(["these","all","are",]));
...
return containsAnyTwice(item,arr2);
})}
else{return "Make sure both the inputs are valid arrays";}
};
console.log(containsAnyTwice([1,2],["hello",1,"hello",1]));
...
var getValuesAppearingTwice = function (arr) {
  if(Array.isArray(arr)){
    ...
    arr.forEach(function(item){
      if(containsTwice2(item,arr)&&!(repeat.indexOf(item)>=0)){
        repeat.push(item);
      }
    })
    return repeat;
...
//     //=> "this
//is a tweet" is not an HTML element!
//
//     mapToTags(5);
//     //=> wat?
//
...
console.log(filterToLol(["lol","LoL","lol","omg","lollerskatea"]));
